...
const slow = (number, difference) => {
  ...
  else if (number > 0) return number - difference;
    ...
    if (accelerationX != undefined && accelerationY != undefined) {
      ...
      balls.forEach((ball, index) => {
        ...
        walls.forEach((wall, wi) => {
          ...
          const wallEnd = {
            x: wall.x + (wall.horizontal ? wall.length : 0),
            y: wall.y + (wall.horizontal ? 0 : wall.length),
          };
          if (wall.horizontal) {
            // Horizontal wall
            if (
              ...
              ball.nextY - ballSize / 2 <= wall.y + wallW / 2 &&
              ball.nextX >= wallStart.x - wallW / 2 &&
              ball.nextX <= wallStart.x + wallW / 2
            ) {
              ...
              if (distance < wallW / 2 + ballSize / 2) {
                if (debugMode)
            ...
            if (
              ...
              ball.nextX - ballSize / 2 <= wall.x + wallW / 2 &&
              ball.nextY >= wallStart.y - wallW / 2 &&
              ball.nextY <= wallStart.y + wallW / 2
            ) {
        ...
        ball.x = ball.nextX;
        ball.y = ball.nextY;
        ballElements[index].style.cssText = `left: ${ball.x}px; top: ${ball.y}px; `;
      });
    }
    // Win detection
    if (
